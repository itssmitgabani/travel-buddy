...
const Payment = () => {
    ...
    const {data , loading} = useFetch(`/payment/airline/${user._id}`)
    const {data1 , loading1} = useFetch1(`/bank/airline/${user._id}`)
    const da = data[0];
  ...
  const handleUpdatePassword = async e => {
    ...
    try{
      ...
      setError(null)
      await axios.post(`${process.env.REACT_APP_BASE_URL}/bank/airline/add`,info)
      setLoading(false)
  ...
  const handleClick = async () =>{
    setLoading(true)
    try{
      axios.put(`${process.env.REACT_APP_BASE_URL}/payment/airline/request/${da && data[0]._id}`)
      setLoading(false)
      window.location.reload()
    }
    catch(err){
